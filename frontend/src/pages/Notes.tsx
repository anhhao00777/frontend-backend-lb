/* ============================================================================ 
 * COMPONENT: GIAO DIỆN QUẢN LÝ GHI CHÚ (Notes.tsx) 
...
// Khai báo kiểu dữ liệu cho Ghi chú
interface NoteItem {
...
function Notes() {
  ...
  ======================================================================== */ 
  const [topic, setTopic] = useState<string>('hoc-tap'); 
  const [notes, setNotes] = useState<NoteItem[]>([]); 
  const [formData, setFormData] = useState<NoteItem>({ id: null, title: '', content: '' }); 
  ...
  const handleSave = () => { 
    ...
    fetch(url, { 
      ...
      .catch(err => console.error("Lỗi khi lưu ghi chú:", err)); 
  }; 

  // Thêm kiểu dữ liệu cho tham số id
  const handleDelete = (id: number | string | null) => { 
    ...
    if (window.confirm('Bạn có chắc muốn xóa ghi chú này?')) { 
      fetch(`http://localhost:5000/api/notes/${topic}/${id}`, { method: 'DELETE' }) 
        .then(() => fetchNotes())
        .catch(err => console.error("Lỗi khi xóa ghi chú:", err)); 
    } 
  }; 

  // Thêm kiểu dữ liệu cho tham số note
  const handleEdit = (note: NoteItem) => setFormData({ id: note.id, title: note.title, content: note.content }); 
